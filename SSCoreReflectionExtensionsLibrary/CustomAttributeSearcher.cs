...
namespace Crestron.SimplSharp.Reflection
	...
	internal abstract class CustomAttributeSearcher<E> where E : class
		...
		//
		public IEnumerable<CustomAttributeData> GetMatchingCustomAttributes (E element, Type optionalAttributeTypeFilter, bool inherit)
			{
		...
		public IEnumerable<CustomAttributeData> GetMatchingCustomAttributes (E element, Type optionalAttributeTypeFilter, bool inherit,
			...
			if (!skipTypeValidation)
				...
				if (!(optionalAttributeTypeFilter.Equals (typeof(Attribute)) || attributeTypeFilterInfo.IsSubclassOf (typeof (Attribute))))
					throw new ArgumentException ("optionAttributeTypeFilter must have a base class of Attribute");
			...
			if (optionalAttributeTypeFilter == null)
				passesFilter = delegate (Type actualType) { return true; };
			else
				{
		...
		private IEnumerable<CustomAttributeData> GetMatchingCustomAttributesIterator (E element, Func<Type, bool> rawPassesFilter, bool inherit)
			...
			foreach (CustomAttributeData cad in GetDeclaredCustomAttributes (element))
				{
				if (passesFilter (/*cad.AttributeType*/ cad.GetType ()))
					{
					yield return cad;
			...
			if (inherit)
				...
				if (element != null)
					...
					//
					Dictionary<Type, AttributeUsageAttribute> encounteredTypes = new Dictionary<Type, AttributeUsageAttribute> (11);

					for (int i = 0; i < immediateResults.Count; i++)
						{
						Type attributeType = immediateResults[i].GetType ();
					...
					do
						...
						foreach (CustomAttributeData cad in GetDeclaredCustomAttributes (element))
							{
							Type attributeType = cad.GetType ();
